...
| `src/sqlWorkspace.ts` | SQL editor bindings, session history, bounded result documents and commands |
| `src/core/sql-workspace.ts` | Read-query boundary, positional parameters, containment and CSV formatting |
| `src/bulkImport.ts` | CSV/JSON import selection, preview, mapping and tracked document mutation |
| `src/core/bulk-import.ts` | Bounded import parsing and atomic guarded row insertion |
...
- **Query-plan runtime**: `src/runtime/query-plan.c` bounds plan compilation and extraction on the primary connection. `node scripts/build-query-plan.mjs` builds the five native libraries and the WASM runtime from that file and pinned sql.js commit `424163d6c9c34acec7bac07eb2cecedd86185a60`. It requires Emscripten 5.0.0 and Zig 0.16.0; use `--emcc PATH` and optionally `--sqljs-source GIT_CHECKOUT`. The source checkout is read through `git archive` at the pin. `scripts/refresh-sqljs.mjs` delegates to this builder. Native Explain uses the primary connection's signalled `Database.allAsync` so cancel RPCs remain deliverable without losing TEMP or pending transaction state. `scripts/refresh-natives.mjs` pins the tested `master` commit, Actions run, and hashes for all five native targets.
- **Runtime artifact gate**: `vendor/query-plan-manifest.json` records source and output hashes. Normal builds verify it before bundling. Regenerate runtime changes with the builder; do not edit native libraries, WASM/glue, or the manifest by hand. `node scripts/build-query-plan.mjs --check` verifies all copies.
