...
import { Upload, Database, FileUp, Github, ArrowLeft, Download, RefreshCw, AlertCircle } from 'lucide-react';
...
export default function DemoPage() {
  ...
  const initializeWorker = useCallback(async (binary: Uint8Array, filename: string) => {
    ...
    // Note: We need to fetch the WASM binary and pass it to the worker
    try {
      const wasmResponse = await fetch('/demo/sql-wasm.wasm');
